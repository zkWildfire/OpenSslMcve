...
This repository generates three executables. The first executable, `Mcve.exe`,
is the original executable that reproduces the bug. This executable demonstrates
the bug by printing the state of all XMM registers before and after calls to
OpenSSL code and is the primary executable that this README refers to. The
second executable, `Tests.exe`, uses the XMM register validation code to test
a wide range of OpenSSL ciphers to determine if any other ciphers are affected
by the same bug. The third executable, `ExampleWaitFor.exe`, attempts to
trigger the bug and cause unexpected behavior in other, non-SSL methods as a
result of the XMM bug.

> The `ExampleWaitFor.exe` executable is currently a work in progress.

Sample output of the Tests executable:
```
...
than code generated by the compiler. The binary will be written to
`_out/[build-type]/Mcve.exe` and can be run with no arguments.

## Context
The x64 calling convention on Windows specifies that registers XMM6-XMM15 are
"non-volatile" or "callee-saved" ([source](https://docs.microsoft.com/en-us/cpp/build/x64-calling-convention?view=msvc-170#callercallee-saved-registers)).
This means that the state of the registers before and after a method call is
required to be the same. However, when the AVX-512 code path is taken in
OpenSSL's poly1305 code, the registers XMM6-XMM15 get zeroed out during the
method call and are not restored to their original values. This is demonstrated
by this test app, which dumps the state of all XMM registers before and after
various calls to OpenSSL to show that the registers are not restored correctly.

A related issue had previously been filed on the OpenSSL GitHub repository
for OpenSSL v1.1.1C. That issue was determined to be caused by the save and
restore registers code only restoring XMM6 and XMM7. The bug that this repo
reproduces is similar, except no registers are restored at all. The link to
the previous, already fixed issue is here:
https://github.com/openssl/openssl/issues/12328
...
## Bug Fix
A .patch file containing modifications to the `poly1305-x86_64.pl` file in the
OpenSSL source code is included in this repository. This patch file adds an
assembly code block to the `$L$blocks_vpmadd52_8x` label that saves XMM6-XMM15
to the stack, plus a corresponding block in the `$L$tail_vpmadd52_8x` code just
before the `$L$no_data_vpmadd52_8x` label that restores the XMM registers from
the stack.

The save and restore blocks added by this patch differ slightly from the
existing save and restore blocks already generated by the perl script. The new
save and restore code blocks use the `rsp` register when saving and restoring
register state as the stack pointer register is not modified during the region
of code that the save and restore blocks surround. As a result, it isn't
necessary to make use of the `r11` register.
...
to see at which point the save code block would be required, I found that most
registers that were used are registers that do not need to be saved. It wasn't
until the `$L$blocks_vpmadd52_8x` section that some of the non-volatile
registers began to be used. It may be possible to rework the code to avoid
using those registers at all, in which case the save and restore blocks would
no longer be necessary. However, this would be a more complex change and is one
I wasn't comfortable making.
...
* Issue appears to be with the `$L$no_data_vpmadd52_8x` label
    ...
    * As a result, the `vzeroall` instruction just above the `$L$no_data_vpmadd52_8x`
      label causes all XMM registers to be zero when the method returns
